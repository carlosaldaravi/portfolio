...
const Stack = ({ stack }: StackProps) => {
  const intl = useIntl();
  const sectionTitle = intl.formatMessage({ id: "page.developer.stack" });
  ...
  return (
    <>
      ...
      <div className="mt-8 grid grid-cols-3 sm:grid-cols-6 lg:grid-cols-7 gap-12 max-w-xl sm:max-w-6xl mx-auto">
        {stack.map((s) => (
          <div key={s.id} className="mx-auto self-center" title={s.name}>
            <SVG
              type={SVG_TYPES[s.svg as keyof typeof SVG_TYPES]}
