...
import { GearItem } from "./gear-card";
...
const Gear = ({ gear }: GearProps) => {
  const [gearToShow, setGearToShow] = useState<GearItem[]>([]);
  useEffect(() => {
    const newArray = gear.map((item) => {
      if (Array.isArray(item[item.name])) {
        return (item[item.name] as GearItem[]).map((i) => i);
      } else {
        return item;
      }
    });
    const flattened = newArray
      .flat()
      .filter((item): item is GearItem => typeof item === "object");
